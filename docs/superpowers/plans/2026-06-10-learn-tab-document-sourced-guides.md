# Learn Tab — Document-Sourced Guides Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite all 15 Lär-tab articles with proper source citations, add 4 new Förarmindset articles, restructure the page with 4 category tabs, and add 2 new AVSAB PDFs to the document corpus.

**Architecture:** Article data is extracted from `page.tsx` to a dedicated `articles.ts` file. `page.tsx` becomes UI-only and reads categories from `articles.ts`. Two new AVSAB PDFs are downloaded and registered in `ingest-docs.ts`. The Article interface gains a `sources` field rendered as a footnote in each expanded card.
...
|--------|------|----------------|
| Create | `src/app/learn/articles.ts` | All article data: Article interface, CATEGORIES const, 19 articles with sources |
| Modify | `src/app/learn/page.tsx` | UI only: tab bar, accordion, `?tab=` routing — no article content |
| Modify | `src/app/learn/page.module.css` | Add tab bar styles + source footnote styles |
| Create | `src/app/learn/articles.test.ts` | Data integrity: every article has sources, IDs unique, categories non-empty |
| Modify | `scripts/ingest-docs.ts` | Add 2 new AVSAB DocSpec entries |
| Add | `docs/training-guides/avsab-punishment-2021.pdf` | Downloaded AVSAB punishment position statement |
| Add | `docs/training-guides/avsab-dominance-2009.pdf` | Downloaded AVSAB dominance theory position statement |

---

## Task 1: Download AVSAB PDFs

**Files:**
- Create: `docs/training-guides/avsab-punishment-2021.pdf`
- Create: `docs/training-guides/avsab-dominance-2009.pdf`

- [ ] **Step 1: Find the AVSAB punishment position statement PDF URL**

WebSearch for: `site:avsab.org punishment position statement PDF`

Expected result: URL ending in `.pdf` on `avsab.org`. The punishment statement was published 2007, updated references to 2021.

- [ ] **Step 2: Download avsab-punishment-2021.pdf**

Use WebFetch on the URL found above and save the binary to `docs/training-guides/avsab-punishment-2021.pdf`.

If WebFetch returns HTML instead of PDF, try the direct AVSAB resources page: `https://avsab.org/resources/position-statements/`

- [ ] **Step 3: Find and download the AVSAB dominance theory PDF**

WebSearch for: `site:avsab.org dominance theory position statement PDF`

Save to `docs/training-guides/avsab-dominance-2009.pdf`.

- [ ] **Step 4: Verify both files exist and are non-empty**

```bash
ls -lh docs/training-guides/avsab-punishment-2021.pdf docs/training-guides/avsab-dominance-2009.pdf
```

Expected: both files, each > 50KB.

- [ ] **Step 5: Commit**

```bash
git add docs/training-guides/avsab-punishment-2021.pdf docs/training-guides/avsab-dominance-2009.pdf
git commit -m "docs: add AVSAB punishment and dominance position statements"
```

---

## Task 2: Register new PDFs in ingest-docs.ts

**Files:**
- Modify: `scripts/ingest-docs.ts`

- [ ] **Step 1: Add the two new DocSpec entries after the existing AVSAB entries**

Open `scripts/ingest-docs.ts`. After the `avsab-puppy-socialization-2024.pdf` entry (around line 88), add:
...
```bash
git add scripts/ingest-docs.ts
git commit -m "feat(ingest): register AVSAB punishment and dominance PDFs"
```

---

## Task 3: Write failing data integrity test

**Files:**
- Create: `src/app/learn/articles.test.ts`

- [ ] **Step 1: Write the test (it will fail because articles.ts doesn't exist yet)**
...
```bash
npx vitest run src/app/learn/articles.test.ts
```
...
## Task 4: Create articles.ts with all article data

**Files:**
- Create: `src/app/learn/articles.ts`

- [ ] **Step 1: Create the file with the Article interface and CATEGORIES**

Create `src/app/learn/articles.ts` with the complete content below. This file contains all 19 articles (15 existing rewritten with sources + 4 new Förarmindset articles).
  ...
  mindset: [
    {
      ...
      sections: [
        ...
        },
        {
          ...
          body: 'Hundar med lättväckt stressrespons (som drar på sig trötthet snabbt) behöver fler vilodagar inbyggda i veckoschema. Det handlar inte om att träna mindre — det handlar om att välja rätt typ av aktivitet för dagen. En lugn sniff-promenad utan krav kan vara mer värdefull än ett misslyckat träningspass.',
        },
        {
    ...
    },
    {
      ...
      sections: [
        ...
        },
        {
          ...
          body: 'AVSAB: "The techniques used to teach manners and skills can strongly affect an animal\'s future behavior and emotional wellbeing." En hund som litar på sin förare lär sig snabbare och generaliniserar bättre. En hund som är osäker på relationen tränar alltid med en del av sin uppmärksamhet riktad mot "är detta säkert?".',
        },
        {
      ...
      sections: [
        {
          ...
          body: 'AVSAB: "Aversive training has been shown to impair dogs\' ability to learn new tasks." Samma gäller för rädslobaserad överstimulering generellt — när ett stimuli överstiger hundens stresströskel går den in i "fight-flight-freeze"-läge. Cortisol och adrenalin tar över. Prefrontal cortex kopplas bort och hunden kan bokstavligen inte tänka.',
        },
        {
...
```bash
git add src/app/learn/articles.ts src/app/learn/articles.test.ts
git commit -m "feat(learn): extract article data to articles.ts with sources field and 4 new mindset articles"
```

---

## Task 5: Refactor page.tsx to use articles.ts + add tab UI
...
git commit -m "refactor(learn): tab UI with ?tab= routing, sourced from articles.ts"
```

---

## Task 6: Add tab bar and footnote CSS

**Files:**
- Modify: `src/app/learn/page.module.css`

- [ ] **Step 1: Append tab bar and source footnote styles to page.module.css**
...
```bash
git add src/app/learn/page.module.css
git commit -m "style(learn): tab bar and source footnote styles"
```
...
- [ ] **Step 2: Open learn page and verify**

Navigate to `http://localhost:3000/learn`.

Check:
- [ ] 4 tabs visible: Valp & Grunder, Träningsteknik, Förarmindset, Beteende
- [ ] Clicking each tab shows correct articles
- [ ] URL updates to `?tab=teknik` etc. on click
- [ ] Expanding an article shows sections + source footnote at bottom
- [ ] Förarmindset tab shows 4 new articles
- [ ] Tab bar scrolls horizontally on narrow screen (resize browser)

- [ ] **Step 3: Stop dev server and commit final state**

If any visual issues were found and fixed in step 2, commit those fixes now.

```bash
git add -p
git commit -m "fix(learn): visual polish from browser verification"
```
